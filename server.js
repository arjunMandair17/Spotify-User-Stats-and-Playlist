...
app.post("/user/data/playlist", async(req,res)=>{
    ...
    let newPlaylist = await fetch(`https://api.spotify.com/v1/me/playlists`, {
        method: "POST",
        headers: {
            "Authorization": `Bearer ${req.session.access_token}`
        },
...
app.get("/user/baseinfo", async (req,res)=>{
    if(!checkAuth(req,res)) return;
...
async function getUserInfo(token){
    ...
    return {name: userInfo.display_name, pfp: userInfo.images[0].url, link: userInfo.external_urls.spotify, images: userInfo.images, followers: userInfo.followers.total};
}

function checkAuth(req,res){
    // checks if the user is authenticated
    if(!req.session.access_token){
        res.status(401).send("Unauthenticated: Sign In");
        return false;
    }
