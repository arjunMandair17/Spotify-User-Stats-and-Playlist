...
// main call of the function, behavior is defined in the helper functions below
document.getElementById("statsButton").addEventListener("click", renderListeningStats);
...
async function renderListeningStats(){
    ...
    artistTable.classList.add("artistTable");
    let topSongsTable = document.createElement("table");
    topSongsTable.classList.add("songsTable");
    ...
    let userBaseInfo = await fetch("user/baseinfo", {
        headers: {
            "Accept": "application/json"
        }
    })
    ...
    // generates a playlist for the user by passing their top 5 songs into reccoBeats API, and adding the recommended songs
...
function generateCard(userData, parentContainer){
    ...
    shareButton.addEventListener("click", async (e)=>{
        ...
        for (const item of userData.songs){
            ...
            let curSongName = document.createElement("td");
            let curSongImg = document.createElement("td");
            let curSongNum = document.createElement("td");
        ...
        for (const item of userData.artists){
            let curRow = document.createElement("tr");
            let curArtistName = document.createElement("td");
            let curArtistImg = document.createElement("td");
            let curArtistNum = document.createElement("td");
            let actualImg = document.createElement("img");
